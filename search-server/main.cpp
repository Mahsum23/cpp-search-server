...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (const auto& word : stop_words)
        {
            if (!IsValidWord(word))
            {
                throw invalid_argument("special symbols" + word);
            }
        }
    }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text.empty()) {
            throw invalid_argument("no word after '-' in query");
        } 
        else if (text[0] == '-') {
            throw invalid_argument("double '-' in query");
        }
        else if (!IsValidWord(text)) {
            throw invalid_argument("special symbols in query");
        }
